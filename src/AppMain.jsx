...
<input type="number"placeholder="96"inputMode="numeric"style={{...inp,background:"#06101a",border:"1px solid #1a3a5c"}}value={form.waterOz}onChange={e=>f("waterOz",e.target.value)}/>
...
{[{l:"Move Cal 🔴",k:"totalActiveCal",p:"1181"},{l:"Exercise Min 🟢",k:"exerciseMin",p:"58"},{l:"Stand Hrs 🔵",k:"standHrs",p:"16"},{l:"Steps",k:"steps",p:"8941"},{l:"Distance mi",k:"distanceMi",p:"4.06"},{l:"Stair Flights",k:"stairFlights",p:"22"}].map(fi=><div key={fi.k}><label style={lbl}>{fi.l}</label><input type="number"placeholder={fi.p}inputMode="decimal"style={inp}value={form[fi.k]}onChange={e=>f(fi.k,e.target.value)}/></div>)}
</div>
<div style={{marginBottom:12}}><label style={lbl}>Stair Load</label><input type="text"placeholder="Backpack ~15lbs + Boots"style={inp}value={form.stairLoad}onChange={e=>f("stairLoad",e.target.value)}/></div>
<div style={{color:"#2a2a3e",fontSize:10,textTransform:"uppercase",marginBottom:10}}>— Workout —</div>
<div style={{marginBottom:12}}><label style={lbl}>Workout Type</label><input type="text"placeholder="Military HIIT"style={inp}value={form.workoutType}onChange={e=>f("workoutType",e.target.value)}/></div>
...
const emptyForm={date:new Date().toISOString().split("T")[0],workoutType:"",workoutDuration:"",workoutCal:"",totalActiveCal:"",exerciseMin:"",standHrs:"",steps:"",distanceMi:"",stairFlights:"",stairLoad:"",waterOz:"",notes:""};
...
const nl=logs.map(d=>d.id===sel.id?updated:d).sort((a,b)=>a.date.localeCompare(b.date));setLogs(nl);setMode("view");setMsg("✓ Day updated!");setTimeout(()=>setMsg(""),2500);}
...
<button onClick={()=>setMode("edit")}style={{background:"none",border:"none",color:"#5ac8fa",fontSize:11,cursor:"pointer",padding:0}}>edit</button>
<button onClick={()=>{if(window.confirm("Delete?"))delDay(sel.id);}}style={{background:"none",border:"none",color:"#2a2a3e",fontSize:11,cursor:"pointer",padding:0}}>delete</button>
</div>
</div>
